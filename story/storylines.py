...
def storyline_suzy_finale(p, sl):
    ...
    if answer == "yes":
        ...
        if not p.has_achievement("suzy_story"): p.unlock_achievement("suzy_story")
        if not p.has_achievement("suzy_romance"): p.unlock_achievement("suzy_romance")
...
def storyline_feelgood_rock_bottom(p, sl):
    ...
    if p.get_balance() >= 300:
        ...
        print()
        answer = ask.option("What do you do?", ["pay", "walk", "threaten"])
        if answer == "pay":
            p.change_balance(-300)
...
def storyline_jameson_rustlers(p, sl):
    ...
    answer = ask.option("What do you do?", ["help", "honk", "watch"])
    if answer == "help":
...
def storyline_lucky_previous_owner(p, sl):
    ...
    answer = ask.option("What do you do?", ["refuse", "prove", "give"])
    if answer == "refuse":
...
def storyline_lockbox_key_hunt(p, sl):
    ...
    answer = ask.option("What do you do?", ["try", "ask", "leave"])
    if answer == "try":  # Try combination
