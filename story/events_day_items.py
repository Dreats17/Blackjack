...
class DayItemsMixin:
    ...
    def item_hoarder(self):
        ...
        inventory_items = [item for item in self._inventory if item != "Car"]
        if inventory_items:
            chosen_item = random.choice(inventory_items)
            type.type("You pick up " + item(chosen_item) + " and smile. You remember where you got it. You remember why you kept it. You remember who gave it to you.")
        print(PAR)
        type.type("Yeah. These aren't just things. They're proof you've lived. Proof you've survived. Proof that you matter. You are so sexy, you hoarder, you.")
    ...
    def shiv_confrontation(self):
        ...
        type.type("Down the street, a man steps out from behind a dumpster. He struts toward you like he's been rehearsing this moment for hours. He probably has.")
        print(PAR)
        type.type(quote("You. Give me your wallet. NOW."))
        print(PAR)
        if self.has_item("Shiv"):
            type.type("Your hand drops to your side, and find the " + cyan(bright("Shiv")) + " tucked in your belt. You pull it out slowly. The duct tape handle catches the orange glow of a streetlight. The blade is modest, ugly, and unconditional.")
            print(PAR)
            ...
            print(PAR)
            type.type("He turns and bolts into the forest. You never said a single word. Your hands are shaking — from adrenaline, mostly — but the " + cyan(bright("Shiv")) + " did exactly what it was made to do.")
            ...
            type.type("PSSSHHHHH!")
            print(PAR)
            type.type("The man screams, both hands clawing at his eyes, stumbling blindly into the forest. You don't bother to follow and see where he goes. One use, one save. The " + cyan(bright("Pepper Spray")) + " is spent, but so is your aggressor.")
            self.use_item("Pepper Spray")
            self.restore_sanity(3)
            print(PAR)
        else:
            type.type("You've got nothing — no shiv, no spray, no bluff good enough to sell. He shoves you hard into the dumpster and freely takes from your pockets with the brisk efficiency of a man on a tight schedule.")
            print(PAR)
            lost = min(random.randint(15, 50), self._balance)
            if lost > 0:
                type.type("He took " + red("${:,}".format(lost)) + " from you.")
                self.change_balance(-lost)
                print(PAR)
                type.type("You sit there for a minute, surrounded by your peers, listening to his footsteps fade, deciding what to feel. I know! You feel like a bag of garbage.")
    ...
    def lockpick_opportunity(self):
        ...
        if self.has_item("Lockpick Set"):
            ...
            if loot[0] == "Cash":
                type.type("Whats inside? Why, " + bright(green("${:,}".format(loot[1]))) + " in crumpled bills. Not bad for a lockbox.")
                print(PAR)
                self.change_balance(loot[1])
            elif not self.has_item(loot[0]):
                type.type("Whats inside? Why, a " + magenta(bright(loot[0])) + "! Finders keepers.")
                self.add_item(loot[0])
                print(PAR)
            else:
                fallback = random.randint(50, 300)
                type.type("Whats inside? Why, " + bright(green("${:,}".format(fallback))) + " and some old receipts. Could be worse.")
                print(PAR)
            ...
            knife_name = "Pocket Knife" if self.has_item("Pocket Knife") else ("Utility Blade" if self.has_item("Utility Blade") else ("Master Knife" if self.has_item("Master Knife") else ""))
            type.type("You try to pry the lock with your " + cyan(bright(knife_name)) + ". ")
            if random.randrange(3) == 0:
                fallback = random.randint(5, 200)
                type.type("After ten minutes of sweating, the lock snaps. What's inside? Why, " + bright(green("${:,}".format(fallback))) + ". Was it worth the effort? Debatable.")
                print(PAR)
                self.change_balance(fallback)
            else:
            ...
            type.type("You walk away, wondering what was inside. Yup. That's going to bother you all day.")
            self.lose_sanity(1)
            print(PAR)
            return
    ...
    def fishing_day(self):
        ...
        type.type("As you wander down the road, you come across a quiet pond tucked behind an abandoned gas station, like nature reclaiming something that didn't deserve to exist. The water is clear enough that you can see shapes moving through it.")
        print(PAR)
        if self.has_item("Fishing Rod"):
            type.type("You grab your " + cyan(bright("Fishing Rod")) + " and cast a line, then sit on the bank and let the world do whatever it's going to do. The water moves. The clouds move. You, for once, don't have to.")
            print(PAR)
            self.restore_sanity(5)
            catch = random.randrange(5)
            if catch == 0:
                type.type("A tug — a real one, not just the current — and you reel in a decent-sized bass. You hold it up and stare at each other for a moment before you wrap it up and put it on ice. Fresh " + cyan(bright("Fish")) + ". Actual food that came from the actual world.")
                self.add_item("Fish")
                print(PAR)
                return
            elif catch == 1:
                type.type("You reel in an old boot. Classic. The fishing gods have a sense of humor that never gets old, apparently. At least the casting was nice. You toss the boot in the dumpster, and eventually, you wander back to the wagon.")
                print(PAR)
                return
            elif catch == 2:
                bonus = random.randint(5, 75)
    ...
    def trap_night_thief(self):
        ...
        if self.has_item("Car Alarm Rigging"):
            ...
            if self.has_item("Flashlight"):
                type.type("You flip on your " + cyan(bright("Flashlight")) + ". ")
            else:
            ...
            print(PAR)
            type.type("Your stuff is safe. Your trap is a mess, but at least you can remake it.")
            self.use_item("Improvised Trap")
    ...
    def slingshot_bird_hunt(self):
        ...
        if self.has_item("Slingshot"):
            ...
            print(PAR)
            type.type("Fire a warning shot, try to hit one, or put it away?")
            choice = ask.option("Your choice?", ["scare", "hit", "away"])
            print(PAR)
            if choice == "scare":
    ...
    def snare_trap_catch(self):
        ...
        if self.has_item("Snare Trap"):
            ...
            if catch == 0:
                ...
                print(PAR)
                choice = ask.option("Your choice?", ["take", "free"])
                print(PAR)
                if choice == "take":
    ...
    def binocular_scope_discovery(self):
        ...
        if self.has_item("Binocular Scope"):
            ...
            if discovery_roll == 0:
                ...
                print(PAR)
                choice = ask.option("Your choice?", ["help", "ignore"])
                print(PAR)
                if choice == "help":
    ...
    def luck_totem_windfall(self):
        ...
        type.type("You wander from the wagon, walking along the painted lines. Nothing special. Same cracks in the asphalt. ")
        if self.has_item("Luck Totem") or self.has_item("Fortune's Favor") or self.has_item("Gambler's Aura"):
            if self.has_item("Gambler's Aura"):
                ...
                used = "Luck Totem"
            type.type("All of a sudden, the " + cyan(bright(used)) + " vibrates in your pocket. It's a good kind of vibration, one that feels good in your soul. Then, you look down.")
            print("\n")
            loot = random.choice([20, 20, 50, 50, 100])
            type.type("A " + bright(green("${:,}".format(loot))) + " bill! On the ground, just sitting there.")
            print("\n")
            ticket = random.randint(5, 50)
            loot += ticket
            type.type("You pick it up. Then you see the scratch ticket under it. My, what luck you've got, huh? You scratch it. " + bright(green("$" + str(ticket))) + ". Winner!")
            print("\n")
            type.type("The " + cyan(bright(used)) + " in your pocket is warm. Things are looking up for yours truly.")
            self.change_balance(loot)
            self.restore_sanity(5)
        else:
            type.type("You step on a piece of gum. It sticks to your shoe. It's the expensive kind that never comes off. JuicyFruit.")
            self.lose_sanity(1)
        print("\n")
        return
    ...
    def lottery_ticket_check(self):
        ...
        print(PAR)
        return
    def bottle_of_tomorrow_use(self):
        """Bottle of Tomorrow — skip to tomorrow with full health and sanity."""
        if not self.has_item("Bottle of Tomorrow"):
            self.day_event()
            return
        type.type("The " + cyan(bright("Bottle of Tomorrow")) + " sits in your hand. The glass is warm. Inside, something swirls like captured sunlight.")
        print(PAR)
        type.type("You uncork it. The smell is impossible to describe — morning dew, fresh coffee, a page turning.")
        print(PAR)
        answer = ask.yes_or_no("Drink it and skip to tomorrow? ")
        if answer == "yes":
            type.type("You drink. The world blurs. Hours fold like origami.")
            print(PAR)
            type.type("When clarity returns, it's evening. You feel rested — not perfect, but steady. The day's weight has lifted.")
            print(PAR)
            type.type("The bottle is empty. Tomorrow is almost here.")
            self.heal(25)
            self.restore_sanity(20)
            self.use_item("Bottle of Tomorrow")
            self._skip_blackjack_tonight = True
        else:
            type.type("Not yet. You cork it and put it back. Tomorrow can wait.")
        print(PAR)

        print(PAR)
        return
    def blank_check_opportunity(self):
        """Blank Check — one free purchase from anywhere."""
        if not self.has_item("Blank Check"):
            self.day_event()
            return
        type.type("You hear a rumor that a traveling merchant has set up behind the gas station.")
        print(PAR)
        type.type("You approach and he sees the " + cyan(bright("Blank Check")) + " in your wallet.")
        print(PAR)
        type.type("His eyes light up. " + quote("That's a Blank Check. You know what that means?"))
        print(PAR)
        type.type(quote("Anything you want. One item. No price. Marvin's signature covers it all."))
        print(PAR)
        premium_items = ["Guardian Angel", "New Identity", "Witch Doctor's Amulet", "Forged Documents", "Dream Catcher"]
        available = [item for item in premium_items if not self.has_item(item)]
        if not available:
            type.type("But you already own everything worth buying. The merchant tips his hat. " + quote("Come back when you need something."))
            print(PAR)
            return
        type.type("He spreads his wares on a blanket:")
        print(PAR)
        for i, item in enumerate(available, 1):
            type.type("  " + str(i) + ". " + cyan(bright(item)))
            print(PAR)
        answer = ask.option("Choose an item (or 'leave'): ", [str(i) for i in range(1, len(available) + 1)] + ["leave"])
        if answer == "leave":
            type.type("You walk away. The Blank Check stays in your wallet. For now.")
        else:
            chosen = available[int(answer) - 1]
            type.type("The merchant wraps the " + cyan(bright(chosen)) + " carefully and hands it to you.")
            print(PAR)
            type.type("He takes the Blank Check. Folds it once. Tucks it into his coat. " + quote("Pleasure doing business."))
            self.add_item(chosen)
            self.use_item("Blank Check")
        print(PAR)
        print(PAR)
        return
