...
class AdventuresMixin:
    ...
    def road_adventure(self):
        ...
        """
        first_visit = not self.has_met("Road Adventure Event")
        self.meet("Road Adventure Event")
        self.add_fatigue(random.randint(5, 10))

        if first_visit and not self.has_item("Road Warrior Badge"):
            type.type("Something catches your eye in the gravel shoulder — a tarnished metal badge, half-buried.")
            print()
            type.type("You brush it off. It reads: " + cyan(bright("ROAD WARRIOR")) + ". Someone who walked this road before you left it behind.")
            print()
            type.type("You pin it to your jacket. The road feels a little less hostile now.")
            self.add_item("Road Warrior Badge")
            self.restore_sanity(10)
            print()

        if self.has_item("Immortal Vehicle"):
            type.type("The " + cyan(bright("Immortal Vehicle")) + " plows through terrain that should destroy it. Not a scratch. Not a squeak.")
            print()
            type.type("Potholes, gravel, broken glass — the vehicle eats it all and asks for more.")
            print()
            type.type("You find a shortcut nobody else could survive. On the other side: abandoned supplies.")
        ...
        if event == "street_dice":
            ...
            action = ask.option("Choose", ["play", "watch", "leave"])
            ...
            if action == "play":
                ...
                if player_result == "instant_win":
                    ...
                    print()
                    again = ask.option("Choose", ["double", "walk"])
                    if again == "double":
                    ...
                    again = ask.option("Choose", ["again", "leave"])
            ...
            print()
            action = ask.option("Choose", ["stop", "honk", "drive_past"])
            ...
            if action == "stop":
                ...
                print()
                action2 = ask.option("Choose", ["drop_her_off", "give_money", "offer_food", "talk_more"])
            ...
            print()
            action = ask.option("Choose", ["touch_stone", "pray", "leave_offering", "read_symbols", "walk_away"])
            ...
            print()
            action = ask.option("Choose", ["help", "sell_water", "entertain", "rob", "ignore"])
            ...
            if action == "help":
                ...
                print()
                price_choice = ask.option("Choose", ["$1", "free", "$5"])
            ...
            print()
            action = ask.option("Choose", ["stop", "toss_food", "follow", "drive_past"])
    ...
    def woodlands_adventure(self):
        ...
        chance.
        """
        first_visit = not self.has_met("Woodlands Adventure Event")
        ...
        if first_visit and not self.has_item("Druid's Staff"):
            type.type("At the edge of the treeline, a gnarled wooden staff leans against an ancient oak. Like it was left here for you.")
            print()
            type.type("You take the " + cyan(bright("Druid's Staff")) + ". The wood hums faintly in your grip. The trees seem to approve.")
            self.add_item("Druid's Staff")
            self.heal(15)
            self.restore_sanity(10)
            print()

        if self.has_item("Assassin's Kit"):
            type.type("The " + magenta(bright("Assassin's Kit")) + " gives you a decisive edge. Blind, strike, move. Professional.")
            print()
            type.type("+75% combat advantage. The woodlands bow to you.")
            self.restore_sanity(8)
        ...
        if self.has_item("Street Fighter Set"):
            type.type("Triple-ingredient advantage: blade cuts, fist breaks. The " + magenta(bright("Street Fighter Set")) + " is a one-person army.")
            print()
            type.type("Adventure boss doesn't stand a chance.")
            self.restore_sanity(8)
            self.change_balance(random.randint(100, 300))
            print()
            return
        ...
        if self.has_item("Beast Tamer Kit"):
            type.type(magenta(bright("Beast Tamer Kit")) + " active. The creatures in this zone treat you as pack leader.")
            print()
            type.type("The woodlands become trivial. Every animal yields.")
            self.restore_sanity(8)
        ...
        if event == "hunting_competition":
            ...
            print()
            action = ask.option("Choose", ["enter", "bet", "observe"])
            ...
            if action == "enter":
                if self.get_balance() >= 5000:
                    ...
                    r1 = ask.option("Choose", ["track", "trap", "climb"])
                    ...
                    r2 = ask.option("Choose", ["rush", "wait", "sabotage"])
            ...
            print()
            action = ask.option("Choose", ["fight", "flee", "offer", "submit"])
            ...
            if action == "fight":
                ...
                attack = ask.option("Choose", ["eyes", "dodge", "dead"])
                ...
                if attack == "eyes":
                    if random.random() < 0.15:
                        ...
                        self.mark_met("Bear King's Respect")
                ...
                flee = ask.option("Choose", ["river", "cliff", "brambles"])
            ...
            type.type("What do you do?")
            print()
            action = ask.option("Choose", ["drink", "wash", "bottle", "leave"])
            ...
            if action == "drink":
                ...
                self.restore_sanity(10)
                print()
                type.type("The water tingles going down. You feel lighter. Younger. More alive.")
                print()
            else:
            ...
            action = ask.option("Choose", ["knock", "peek", "leave"])
            ...
            if action == "knock":
                ...
                if answer == "yes":
                    ...
                    cost = ask.option("Choose", ["memory", "year", "favor"])
                    if cost == "memory":
            ...
            if self.has_item("Signal Mirror"):
                ...
                self.earn_money(found)
                self.restore_sanity(5)
            if self.has_item("Smoke Signal Kit"):
                print()
                type.type("You assemble your " + cyan(bright("Smoke Signal Kit")) + " and light the compressed fuel. Thick white smoke billows up through the trees.")
                print()
                type.type("A search helicopter spots it within minutes. They circle overhead and drop a supply cache — emergency rations and a first aid kit.")
                print()
                type.type("The cache contains medical supplies worth treating several wounds, plus some packaged food.")
                self.heal(15)
                self.restore_sanity(8)
    ...
    def swamp_adventure(self):
        ...
        """
        first_visit = not self.has_met("Swamp Adventure Event")
        ...
        if first_visit and not self.has_item("Swamp Rune"):
            type.type("Half-submerged in the muck, a stone glows faintly green. You reach in and pull it free.")
            print()
            type.type("A " + cyan(bright("Swamp Rune")) + ". The carving is old — older than the swamp itself. It feels like a ward against poison.")
            self.add_item("Swamp Rune")
            self.restore_sanity(8)
            print()
        ...
        if event == "tortoise_racing":
            ...
            action = ask.option("Choose", ["race", "bet", "watch"])
            ...
            if action == "race":
                if self.get_balance() >= 2000:
                    ...
                    r1 = ask.option("Choose", ["lettuce", "yelling", "poking"])
                    ...
                    r2 = ask.option("Choose", ["cheer", "pray", "throw"])
                ...
                turtle = ask.option("Pick a number", ["1", "2", "3", "4"])
            ...
            print()
            action = ask.option("Choose", ["fight", "bribe", "riddle", "run"])
            ...
            if action == "fight":
                ...
                attack = ask.option("Choose", ["kneecaps", "climb", "distract"])
            ...
            print()
            action = ask.option("Choose", ["free", "keep", "negotiate", "ignore"])
            ...
            if action == "free":
                ...
                for i in range(3):
                    ...
                    wish = ask.option("Choose", ["money", "luck", "health", "item", "info"])
                    if wish == "money":
                ...
                if trust == "yes":
                    ...
                    wish = ask.option("Choose", ["money", "luck", "health"])
                    if wish == "money":
            ...
            if not self.has_achievement("mermaid_met"): self.unlock_achievement("mermaid_met")
            ...
            print()
            action = ask.option("Choose", ["kiss", "talk", "run", "insult"])
            ...
            print()
            action = ask.option("Choose", ["wrestle", "bet", "watch", "nope"])
            ...
            if action == "wrestle":
                if self.get_balance() >= 1000:
                    ...
                    r1 = ask.option("Choose", ["circle", "charge", "taunt"])
                    ...
                    r2 = ask.option("Choose", ["jaw_clamp", "roll", "escape"])
    ...
    def beach_adventure(self):
        first_visit = not self.has_met("Beach Adventure Event")
        ...
        if first_visit and not self.has_item("Sea Glass"):
            type.type("The tide pulls back and leaves something behind. A piece of glass, tumbled smooth by the ocean, glowing pale blue in the moonlight.")
            print()
            type.type("You pick up the " + cyan(bright("Sea Glass")) + ". It's cool to the touch. Calming. The waves seem to whisper: welcome.")
            self.add_item("Sea Glass")
            self.restore_sanity(12)
            print()

        if self.has_item("Hydration Station"):
            type.type("The " + magenta(bright("Hydration Station")) + " collects dew at dawn. You have water when everyone else is parched.")
            print()
            type.type("Desert zone advantage. You thrive while others wilt.")
            self.restore_sanity(8)
            self.heal(10)
            print()
            return
        ...
        if self.has_item("All-Weather Armor"):
            type.type("The " + magenta(bright("All-Weather Armor")) + " laughs at the conditions. Swamp, desert, tundra — you adapt to everything.")
            print()
            type.type("Zone immunity. The environment can't touch you.")
            self.restore_sanity(8)
            self.heal(10)
            print()
            return
        ...
        if self.has_item("SOS Kit"):
            type.type(magenta(bright("SOS Kit")) + " fires. Rangers spot you from their tower. You're pulled out within the hour.")
            print()
            type.type("Emergency adventure exit. Safe and sound.")
            self.restore_sanity(5)
            print()
            return
        ...
        if event == "volleyball_tournament":
            ...
            print()
            action = ask.option("Choose", ["join", "bet", "watch", "nope"])
            ...
            if action == "join":
                if self.get_balance() >= 3000:
                    ...
                    m1 = ask.option("Choose", ["bump", "spike", "dive"])
                    ...
                    m2 = ask.option("Choose", ["block", "set", "cheer"])
                    ...
                    m3 = ask.option("Choose", ["trust_grandma", "go_hero", "teamwork"])
                ...
                pick = ask.option("Pick a number", ["1", "2", "3", "4"])
            ...
            print()
            action = ask.option("Choose", ["join", "observe", "sabotage", "leave"])
            ...
            if action == "join":
                ...
                focus = ask.option("Choose", ["wealth", "love", "power", "peace"])
            ...
            print()
            action = ask.option("Choose", ["open", "shake", "throw_back", "sell"])
            ...
            action = ask.option("Choose", ["race", "bet", "catch_own", "watch"])
            ...
            if action == "race":
                if self.get_balance() >= 500:
                    ...
                    motivate = ask.option("Choose", ["yelling", "food", "poking", "singing"])
            ...
            print()
            action = ask.option("Choose", ["enter", "judge", "sabotage", "watch"])
            ...
            if action == "enter":
                if self.get_balance() >= 200:
                    ...
                    style = ask.option("Choose", ["classic_castle", "modern", "weird", "huge"])
    ...
    def underwater_adventure(self):
        first_visit = not self.has_met("Underwater Adventure Event")
        ...
        if first_visit and not self.has_item("Depth Charm"):
            type.type("Something catches the light below — a small charm tangled in kelp, encrusted with barnacles but still shining.")
            print()
            type.type("You free the " + cyan(bright("Depth Charm")) + " and clip it to your belt. It pulses with a faint warmth, like a heartbeat from the deep.")
            self.add_item("Depth Charm")
            self.change_balance(random.randint(100, 300))
            self.restore_sanity(8)
            print()
        ...
        if event == "hunting_competition":
            ...
            print()
            action = ask.option("Choose", ["compete", "bet", "sabotage", "watch"])
            ...
            if action == "compete":
                if self.get_balance() >= 5000:
                    ...
                    print()
                    target = ask.option("Choose", ["grouper", "tuna", "marlin"])
                    ...
                    shark = ask.option("Choose", ["fight", "hide", "bait", "flee"])
                    ...
                    final = ask.option("Choose", ["deep_dive", "surface_hunt", "ambush"])
                ...
                sabotage = ask.option("Choose", ["airhose", "steal", "shark"])
            ...
            print()
            action = ask.option("Choose", ["explore", "quick_loot", "photograph", "leave"])
            
            if action == "explore":
                type.type("You descend carefully toward the wreck. The wood creaks even underwater.")
                print()
                type.type("Where do you explore first?")
                area = ask.option("Choose", ["captains_quarters", "cargo_hold", "deck", "hull"])
                ...
                if area == "captains_quarters":
                    ...
                    if random.random() < 0.5:
                        ...
                        escape = ask.option("Choose", ["break_hull", "signal", "calm"])
                ...
                sell = ask.option("Choose", ["museum", "hunter"])
                if sell == "museum":
            ...
            if self.has_item("Animal Whistle") and not self.has_companion("Kraken"):
                ...
                self.increment_statistic("companions_befriended")
                self.unlock_achievement("first_friend")
                self.unlock_achievement("kraken_encounter")
            ...
            print()
            action = ask.option("Choose", ["fight", "communicate", "flee", "offer"])
            ...
            if action == "fight":
                ...
                attack1 = ask.option("Choose", ["dodge", "cut", "grab"])
                ...
                attack2 = ask.option("Choose", ["eyes", "throat", "retreat"])
            ...
            print()
            action = ask.option("Choose", ["accept", "decline", "ask_questions", "steal"])
            ...
            print()
            action = ask.option("Choose", ["dive", "hire_help", "sell_map", "abandon"])
            ...
            if action == "dive":
                ...
                eel = ask.option("Choose", ["fight_eel", "distract", "sneak"])
    ...
    def city_adventure(self):
        first_visit = not self.has_met("City Adventure Event")
        ...
        if first_visit and not self.has_item("Underground Pass"):
            type.type("A street kid with hollow eyes presses something into your hand as you pass. A laminated card.")
            print()
            type.type("An " + cyan(bright("Underground Pass")) + ". Access to places the city pretends don't exist. The kid vanishes into the crowd.")
            self.add_item("Underground Pass")
            self.restore_sanity(5)
        ...
        if event == "underground_den":
            ...
            print()
            action = ask.option("Choose", ["play", "watch", "help_woman", "leave"])
            ...
            print()
            action = ask.option("Choose", ["call_911", "help", "narcan", "leave"])
            ...
            print()
            action = ask.option("Choose", ["borrow", "refuse", "threaten", "ask_about_him"])
            ...
            print()
            action = ask.option("Choose", ["sit", "give_money", "share_food", "ask_advice", "leave"])
            ...
            if self.has_item("Intelligence Dossier"):
                ...
                print()
                action = ask.option("Choose", ["lost", "curious", "buy", "run"])
            ...
            print()
            action = ask.option("Choose", ["play_along", "be_honest", "steal", "leave"])
            ...
            print()
            action = ask.option("Choose", ["give_money", "give_advice", "take_picture", "ignore"])
            ...
            print()
            action = ask.option("Choose", ["join", "report", "blackmail", "fight", "watch"])
            ...
            if action == "join":
                ...
                grab = ask.option("Choose", ["reasonable", "greedy", "pocket_stuff"])
            ...
            print()
            action = ask.option("Choose", ["get_ice_cream", "suspicious", "follow_truck", "ignore"])
            ...
            if action == "get_ice_cream":
                ...
                flavor = ask.option("Choose", ["chocolate", "vanilla", "mystery"])
                ...
                if random.random() < 0.3:
                    ...
                    choice = ask.option("Choose", ["report", "buy"])
                    if choice == "buy":
            ...
            print()
            action = ask.option("Choose", ["fight", "bet", "watch", "organize"])
            ...
            if action == "fight":
                if self.get_balance() >= 2000:
                    ...
                    r1 = ask.option("Choose", ["duck", "block", "counter"])
                    ...
                    r2 = ask.option("Choose", ["knee", "headbutt", "trip"])
                    ...
                    r3 = ask.option("Choose", ["all_in", "defensive", "taunt"])
            ...
            print()
            action = ask.option("Choose", ["fight", "negotiate", "surrender", "distraction", "run"])
            
            if action == "fight":
                type.type("You're not going down without a fight!")
                print()
                type.type("How do you attack?")
                attack = ask.option("Choose", ["leader", "closest", "wild_swing"])
            ...
            print()
            action = ask.option("Choose", ["join", "refuse", "betray", "negotiate"])
            ...
            if action == "join":
                ...
                distraction = ask.option("Choose", ["fight", "fire_alarm", "drunk_act", "spill"])
